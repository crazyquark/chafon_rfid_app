...
class rfid_app(serial.Serial):
    ...
    def __init__(self, dev_path, bauds=38400, debug=0):
        ...
        serial.Serial.__init__(self, dev_path, bauds, 8,
                               serial.PARITY_NONE, timeout=0)
        self._debug = debug
    ...
    @staticmethod
    def _strtohex(s, sep=" "):
        return sep.join(map(lambda c: "%02x" % (ord(c),), s))
    ...
    def _hextostr(s):
        ...
        for c in s:
            ...
            if c == ' ':
                ...
                raise ValueError("not an hexadecimal character: %c" % c)
            ...
            if tmp == None:
                ...
                tmp = None
        if tmp != None:
            raise ValueError("even number of hexadecimal characters")
        return hex
    ...
    def _execute_waitresult(self, opcode, data="", timeout=10, check_result=None):
        ...
        cmd = "\xAA\xDD\x00" + chr(len(opcode_data) + 1) + \
            opcode_data + self._checksum(opcode_data)

        if self._debug:
            print >> sys.stderr, "send %s" % (self._strtohex(cmd),)
        self.write(cmd)
        time.sleep(0.1)
        ...
        if self._debug:
            print >> sys.stderr, "recv %s" % (self._strtohex(result),)
        ...
        if self._checksum(result[4:]) != "\x00":
            raise IOError("answer bad checksum: %s" %
                          (self._strtohex(result),))
    ...
    def beep(self, duration=10):
        ...
        self._execute_waitresult("\x01\x03", chr(
            duration), check_result=ERR_NONE)
...
if __name__ == "__main__":
    ...
    group.add_argument('-i', '--info', action="store_true",
                       help="reader information")
    parser.add_argument('-d', '--device', required=True,
                        help="path to serial communication device")
    parser.add_argument('-v', '--verbose', action="store_true",
                        help="trace serial commands")
    parser.add_argument('-l', '--lock', action="store_true",
                        help="attempt to lock tag")
    parser.add_argument(
        '-t', '--type', choices=["hex", "dec"], default="hex", help="input/output type (hex, dec)")
    ...
    if args.info:
        ...
        dev.write_token_raw(rfid_app._hextostr(args.data), args.lock)
